...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->test->test->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($conf->test->enabled)) accessforbidden('Module not enabled');
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/test/test_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/test/test_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
